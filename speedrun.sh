...
NPN=8
torchrun --standalone --nproc_per_node=$NPN -m scripts.base_train -- --depth=20 --target_param_data_ratio=20 --run="$WANDB_RUN"
...
torchrun --standalone --nproc_per_node=$NPN -m scripts.mid_train -- --run=$WANDB_RUN
...
# torchrun --standalone --nproc_per_node=$NPN -m scripts.chat_rl -- --run=$WANDB_RUN
# eval the RL model only on GSM8K
# torchrun --standalone --nproc_per_node=$NPN -m scripts.chat_eval -- -i rl -a GSM8K
