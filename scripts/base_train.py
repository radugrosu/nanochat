...
build_val_loader = partial(
    tokenizing_distributed_data_loader, device_batch_size, max_seq_len, split="val", device=device
)
...
for step in range(num_iterations + 1):
    ...
    # logging
    smooth_train_loss = (
        ema_beta * smooth_train_loss + (1 - ema_beta) * train_loss.item()  # type: ignore
    ...
    print0(
        ...
        f" tok/sec: {tok_per_sec:,} | mfu: {mfu:.2f} | total time: {total_training_time / 60:.2f}m"
    )
    if step % 100 == 0:
        wandb_run.log(
            {
...
get_report().log(
    ...
    data=[
        ...
        {  # stats about training outcomes
            ...
            "Total training flops": f"{flops_so_far:e}",  # type: ignore
