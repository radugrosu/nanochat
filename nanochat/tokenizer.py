...
from typing import Iterable, TypeAlias, cast, overload
...
from rustbpe import Tokenizer as RbpeTokenizer
...
Content: TypeAlias = str | list[ContentDict]


class SystemMessage(TypedDict):
    role: Literal["system"]
    content: str  # adjust if your system can also be list[ContentDict]


class UserMessage(TypedDict):
    role: Literal["user"]
    content: str  # <- user is plain string
...
class Conversation(TypedDict):
    # if you really require a list, use list[Message]
...
class HuggingFaceTokenizer:
    ...
    @classmethod
    def from_pretrained(cls, hf_path: FilePath):
        # init from a HuggingFace pretrained tokenizer (e.g. "gpt2")
        tokenizer = HFTokenizer.from_pretrained(hf_path)
        tokenizer = cast(HFTokenizer, tokenizer)
        return cls(tokenizer)

    @classmethod
    def from_directory(cls, tokenizer_dir: FilePath):
    ...
    def _encode_one(
        ...
        encodings = self.tokenizer.encode(text, add_special_tokens=False)
        ids.extend(encodings.ids)  # type: ignore
    ...
    def save(self, tokenizer_dir: FilePath):
...
class RustBPETokenizer:
    ...
    @classmethod
    def from_pretrained(cls, tiktoken_name: str):
    ...
    def encode(
        ...
        *,
        ...
        if isinstance(text, str):
            ...
            assert isinstance(text, list), f"Invalid input type: {type(text)}"
    ...
    def render_conversation(
        ...
        def add_tokens(token_ids: list[int] | int, mask_val: int):
        ...
        if first["role"] == "system":
            ...
            second = messages[1]
            assert second["role"] == "user", "System message must be followed by a user message"
    ...
    def visualize_tokenization(self, ids: list[int], mask: list[int]) -> str:
    ...
    def render_for_completion(self, conversation: Conversation) -> list[int]:
        """
